<template>
 <div class="flex flex-col">
     ...
     <ul class="flex flex-col mt-3 list-none">
         ...
         <span
           :class="filter.active ? `text-${filter.colors}` : 'text-brand-graydark'"
           class="font-bold"
         >
             {{ filter.amount }}
         </span>
...
function applyFiltersStructure (summary) {
  return Object.keys(summary).reduce((Anterior, Atual) => {
    const currentFilter = {
      label: Labels[Atual],
      colors: Colors[Atual],
      amount: summary[Atual]
    }

    if (Atual === 'all') {
      currentFilter.active = true
    } else {
      currentFilter.type = Atual
    }
    return [...Anterior, currentFilter]
  }, [])
}
export default {
