    ...
    <teleport to='body'>
        <div v-if="state.isActive" class="fixed top-0 left-0 z-50 flex items-center justify-center w-full h-full bg-black bg-opacity-50" @click="handleModalToogle({status: false})">
            <div class="fixed mx-10" :class="state.width" @click.stop>
                <div class="flex flex-col overflow-hideden bg-white rounded-lg animate__animated animate__fadeInDown animate__faster">
...
const ModalCreate = defineAsyncComponent(() => import('../modalCreate'))
const DEFAULT_WIDTH = 'w-3/4 lg:w-1/3'
export default {
  components: { ModalLogin, ModalCreate },
  setup () {
